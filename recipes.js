...
// Get recipe by foodId
router.get('/:foodId', async (req, res) => {
  try {
    const recipe = await Recipe.findOne({ foodId: req.params.foodId });
...
// Create or update recipe by foodId
router.post('/:foodId', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Unable to save recipe' });
  }
});

module.exports = router;

// Seed missing recipes (one-off helper)
router.post('/seed', async (req, res) => {
  try {
    ...
    for (const f of foods) {
      const exists = await Recipe.findOne({ foodId: f._id });
      if (exists) continue;
